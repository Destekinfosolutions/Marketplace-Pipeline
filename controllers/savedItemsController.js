...
const saveItem = asyncHandler(async (req, res) => {
  ...
  const data = {
    ...
    image:
      `https://marketplaceaws.waayupro.in/uploads/images/product/${product?.productImage[0]}` ||
      "",
  ...
  const data = await Promise.all(
    items.map(async (item) => {
      ...
      return {
        ...
        image: `https://marketplaceaws.waayupro.in/uploads/images/product/${item.productId.productImage[0]}`,

...
const getHomePageData = asyncHandler(async (req, res) => {
  ...
  const transformedCategories = category.map((category) => ({
    ...
    image: `https://marketplaceaws.waayupro.in/uploads/images/category/${category.categoryImage}`,
  }));

  const vendor = await Vendor.find().limit(3);
  ...
  res.status(200).json({
    sliderArray: [
      {
        image:
          "https://marketplaceaws.waayupro.in/uploads/images/banner/banner.png",
      },
      {
        image:
          "https://marketplaceaws.waayupro.in/uploads/images/banner/banner.png",
...
const maintenaceData = asyncHandler(async (req, res) => {
  ...
  subCategories.forEach((subCategory) => {
    ...
    subCategoriesByCategory.get(subCategory?.categoryId?.categoryName).push({
      ...
      image: `https://marketplaceaws.waayupro.in/uploads/images/subCategory/${subCategory.subCategoryImage}`,
    });
  });
...
const getBanners = asyncHandler(async (req, res) => {
  const img1 = `https://marketplaceaws.waayupro.in/uploads/images/banner/MarketplaceBanner-01.jpg`;
  const img2 = `https://marketplaceaws.waayupro.in/uploads/images/banner/MarketplaceBanner-02.jpg`;
